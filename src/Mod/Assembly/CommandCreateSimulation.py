...
class Simulation:
    def __init__(self, feaPy):
        ...
        if not hasattr(feaPy, "Motions"):
            feaPy.addProperty(
                "App::PropertyLinkList",
                "Motions",
                "Simulation",
                "Motions objects of the simulation.",
            )
    ...
    def onChanged(self, feaPy, prop):
        if prop == "Motions" and hasattr(self, "motionsChangedCallback"):
...
class ViewProviderMotion:
    def __init__(self, vp):
        assert isinstance(vp, Gui.ViewProviderDocumentObject), "Type error"
        vp.Proxy = self
        self.updateLabel()
    ...
    def updateLabel(self):
        ...
        label = self.app_obj.Joint[0].Label + "("
        if self.app_obj.MotionType == "Linear":
            label = label + translate("Assembly", "Linear")
        else:
            label = label + translate("Assembly", "Angular")
        label = label + ")"
        self.app_obj.Label = label
...
SLOPE defines the steepness of the transition between 0 and H1 and H2 to 0 about time = T1 and T2 respectively. Higher values gives sharper cornered impulses. SLOPE = 1000 or greater are suitable.""",
        ...
        def toggle_help():
            if self.help_label1.isVisible():
                ...
                self.help_label0.setVisible(True)
                self.help_label1.setVisible(True)
                self.help_label2.setVisible(True)
                self.help_label3.setVisible(True)
                self.help_label4.setVisible(True)
                self.help_label5.setVisible(True)
                self.help_label6.setVisible(True)
                self.help_label7.setVisible(True)
    ...
    def positionDialog(self):
        ...
        # Set the position of the dialog at the mouse pointer
        main_window = Gui.getMainWindow()

        # Map the cursor position to the application's main window
        global_position = main_window.mapFromGlobal(self.initialPos)

        # Adjust dialog position to ensure it stays within the application window
        ...
        if dialog_position.x() + self.dialog.width() > screen_geometry.width():
            dialog_position.setX(screen_geometry.width() - self.dialog.width())
        if dialog_position.y() + self.dialog.height() > screen_geometry.height():
            dialog_position.setY(screen_geometry.height() - self.dialog.height())

        # Ensure the dialog does not go above or to the left of the screen
        if dialog_position.x() < 0:
            dialog_position.setX(0)
        if dialog_position.y() < 0:
            dialog_position.setY(0)
...
class TaskAssemblyCreateSimulation(QtCore.QObject):
    ...
    def onItemDoubleClicked(self, item):
        ...
        if row < len(self.simFeaturePy.Motions):
            motion = self.simFeaturePy.Motions[row]
    ...
    def createMotionObject(self, motionType, joint, formula):
        ...
        listOfMotions = self.simFeaturePy.Motions
        listOfMotions.append(motion)
        self.simFeaturePy.Motions = listOfMotions

    def onMotionsChanged(self):
        self.form.motionList.clear()
        for motion in self.simFeaturePy.Motions:
    ...
    def deleteSelectedMotions(self):
        ...
        for index in sorted_indexes:
            ...
            if row < len(self.simFeaturePy.Motions):
                ...
                self.simFeaturePy.Motions.remove(motion)
                # Delete the object
